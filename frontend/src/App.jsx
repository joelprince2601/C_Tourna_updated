...
import { apiGetJson, apiPostFormJson, apiGetBlob, apiDeleteJson, API_BASE } from "./lib/api.js";
...
function App() {
  ...
  const loadClips = async () => {
    try {
      ...
      const toFetch = (response.clips || []).filter(c => c.clip_id && !clipPreviews[c.clip_id]);
  ...
  const handleMarkEvent = async () => {
    if (mode === 'single') {
      ...
      if (!leftVideoRef.current || !rightVideoRef.current || !leftVideoFile || !rightVideoFile) {
    ...
    const timestamp = mode === 'single' ? (videoRef.current?.currentTime || 0) : (leftVideoRef.current?.currentTime || 0);
    ...
    if (mode === 'switch') {
      ...
      const normalized = switchPlan.map(seg => {
        cumulative = Math.max(cumulative, 0) + Math.max(0, Number(seg.till || 0) - cumulative);
        return { view: seg.view, till: cumulative };
      });
      // Cap last till at total duration
      if (normalized.length > 0 && normalized[normalized.length - 1].till > Number(switchTotalDuration)) {
    ...
    try {
      ...
      setEventLog(prev => prev.map(entry =>
        entry.id === logId
      ...
      if (err.status === 507) {
        ...
        alert("⚠️ Disk space is running low! Please free up at least 300MB and try again.");
  ...
  const handleGenerateHighlights = async () => {
    ...
    const processingClips = eventLog.filter(entry => entry.status === 'processing');
    if (processingClips.length > 0) {
      const proceed = confirm(`${processingClips.length} clip(s) are still processing. Generate highlights with saved clips only?`);
      if (!proceed) return;
    }
    ...
    try {
      ...
      if (err.status === 507) {
        ...
        errorMessage = "Request timeout - video processing took too long. Try with fewer clips.";
      }
  ...
  const handleClearClips = async () => {
    ...
    if (!confirm("Are you sure you want to clear all saved clips and event log?")) {
      return;
    }
    ...
    <div className="app-container">
      ...
      <div className="main-content">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button
            ...
            className="btn"
            style={{ backgroundColor: mode === 'single' ? '#2563eb' : '#374151', color: 'white' }}
          >
            🎥 Single View
          </button>
          <button
            ...
            className="btn"
            style={{ backgroundColor: mode === 'switch' ? '#2563eb' : '#374151', color: 'white' }}
          >
            🔀 Switch Mode
          </button>
          <button
            ...
            className="btn"
            style={{ backgroundColor: mode === 'slider' ? '#2563eb' : '#374151', color: 'white' }}
          >
            🎬 Slider Mode
          </button>
          <button
            ...
            className="btn"
            style={{ backgroundColor: mode === 'aspect' ? '#2563eb' : '#374151', color: 'white' }}
          >
        ...
        {mode === 'slider' ? (
          ...
          <div>
              ...
              <h2 style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: '0.5rem', color: 'white' }}>
                📐 Aspect Video Changer
                    ...
                    <p style={{ color: '#9ca3af', marginBottom: '1rem', fontSize: '0.9rem' }}>
                        Convert your video to 1:1 aspect ratio with 5:4 crop ratio. The video will be cropped to 5:4 (center crop) and stretched to fill a 1:1 frame.
                    </p>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const response = await fetch(`${API_BASE}/convert_aspect`, {
                        method: "POST",
                        headers: {
                          'X-Session-Id': sessionId
                        },
                      ...
                      if (!contentType || !contentType.includes("video")) {
                        ...
                        throw new Error(`Expected video file, got ${contentType}: ${errorText}`);
                      }
                      ...
                      alert("🎉 Video converted successfully! You can now download it.");
                    } catch (err) {
                ...
                >
                  {isConverting ? (
                    ...
                    '🔄 Convert to 1:1 (5:4 Crop)'
                        ...
                        <p className="instructions-text" style={{ marginTop: '0.5rem' }}>
                            Click the button to convert your video. The video will be cropped to 5:4 (center crop) and stretched to fill a 1:1 frame (1920x1920).
                        </p>
              </div>
            )}
            ...
            {/* Converted Video */}
            {convertedVideoURL && (
              <div className="clip-section" style={{
                ...
                <p style={{
                  ...
                  fontSize: '1.1rem'
                    ...
                    Your video has been converted to 1:1 aspect ratio with 5:4 crop ratio. Download it below!
                </p>
                ...
                <div style={{
                  ...
                  <button
                    onClick={() => {
                      const video = document.querySelector('.clip-section video');
                      if (video) {
                        video.currentTime = 0;
                        video.play();
                      }
          ...
          <>
            ...
            {mode === 'single' && previewURL && (
              <div className="video-section">
                ...
                <p className="instructions-text">
                  Play the video and click the button at the moment you want to capture.
                  Each clip will be saved for the final highlights video.
                </p>
              </div>
            )}
            ...
            {mode === 'switch' && (leftPreviewURL || rightPreviewURL) && (
              <div className="video-section">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                  <div className="video-container">
                    {leftPreviewURL ? (
                      ...
                      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>No left video</div>
                    )}
                  </div>
                  <div className="video-container">
                    {rightPreviewURL ? (
                      ...
                      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>No right video</div>
                    )}
                  </div>
                </div>
                ...
                <p className="instructions-text">
                  Play both videos in sync. Clicking will save a clip that switches views per your plan.
                </p>
                ...
                {mode === 'switch' && (
                  <div style={{ marginTop: '0.75rem', backgroundColor: '#111827', border: '1px solid #374151', borderRadius: '8px', padding: '0.75rem' }}>
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '0.5rem' }}>
                      ...
                      <input type="number" min="1" value={switchTotalDuration}
                             onChange={(e) => setSwitchTotalDuration(Number(e.target.value) || 1)}
                    ...
                    {switchPlan.map((seg, idx) => (
                      <div key={idx} style={{ display: 'grid', gridTemplateColumns: '160px 1fr 100px', gap: '0.5rem', alignItems: 'center', marginBottom: '0.5rem' }}>
                        <select value={seg.view} onChange={(e) => {
                          const v = e.target.value;
                          setSwitchPlan(prev => prev.map((s, i) => i === idx ? { ...s, view: v } : s));
                        }} className="file-input">
                          <option value="left">Left</option>
                          <option value="right">Right</option>
                        </select>
                        <div style={{ color: '#9ca3af' }}>till</div>
                        <input type="number" min="0" value={seg.till}
                               onChange={(e) => {
                                 const v = Number(e.target.value) || 0;
                                 setSwitchPlan(prev => prev.map((s, i) => i === idx ? { ...s, till: v } : s));
                               }} className="file-input" />
                        <button onClick={() => setSwitchPlan(prev => prev.filter((_, i) => i !== idx))}
                                className="btn" style={{ backgroundColor: '#7f1d1d', color: 'white', gridColumn: '1 / span 3' }}>Remove</button>
                      </div>
                    ))}
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button onClick={() => setSwitchPlan(prev => [...prev, { view: 'left', till: (prev[prev.length-1]?.till || 0) + 5 }])}
                              className="btn" style={{ backgroundColor: '#374151', color: 'white' }}>+ Add Segment</button>
                      <button onClick={() => setSwitchPlan([{ view: 'left', till: 5 }, { view: 'right', till: 10 }])}
                ...
                <div style={{
                  ...
                  {eventLog.map((entry) => (
                    <div key={entry.id} style={{
                      ...
                      padding: '0.5rem',
                      marginBottom: '0.5rem',
                      backgroundColor: entry.status === 'saved' ? '#065f46' :
                                     entry.status === 'error' ? '#7f1d1d' : '#374151',
                      borderRadius: '4px',
                      fontSize: '14px'
                    }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        {entry.status === 'saved' && (
                          <span style={{ color: '#10b981' }}>✅ Saved (ID: {entry.clipId})</span>
                        )}
                        {entry.status === 'error' && (
              ...
              <div className="clip-section">
                ...
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                  <button
                    ...
                    className="btn btn-success"
                    style={{
                      backgroundColor: isGeneratingHighlights ? '#4b5563' : '#059669',
                      cursor: isGeneratingHighlights ? 'not-allowed' : 'pointer'
                    }}
                  >
                  ...
                  {clips.map((clip) => {
                    ...
                    return (
                      <div key={clip.clip_id} style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#9ca3af', marginBottom: '0.5rem', fontSize: '0.9rem' }}>
                          <span>Clip ID: {clip.clip_id}</span>
                          {clip.mode && (
                            <span style={{ backgroundColor: '#374151', color: 'white', padding: '2px 6px', borderRadius: '4px', fontSize: '0.75rem' }}>
                              {clip.mode === 'switch' ? 'Switch' : 'Single'}
                            </span>
                          )}
                        </div>
                        <div style={{ width: '100%', backgroundColor: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px' }}>
                          {url ? (
                            <video
                              src={url}
                              controls
                              muted
                              className="clip-video"
                              style={{ width: '100%', height: 'auto', maxHeight: '70vh', objectFit: 'contain', border: '1px solid #374151', borderRadius: '4px' }}
                            />
                          ) : (
                            <span style={{ color: '#6b7280' }}>Processing...</span>
                          )}
                        </div>
                        <div style={{ color: '#9ca3af', marginTop: '0.5rem', fontSize: '0.85rem' }}>
                          {`Range: ${Math.floor(clip.start_time)}s - ${Math.floor(clip.end_time)}s`}
                        </div>
                      </div>
                    );
                  })}
      ...
      <div className="instructions-section">
        ...
        <ol className="instructions-list">
          ...
          <li>3. Click "Mark Event & Save Clip" for each highlight (±10s clips)</li>
