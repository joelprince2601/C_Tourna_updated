...
class FFmpegService:
    ...
    def add_text_overlay(
        ...
        Args:
            ...
            font_color: Text color (e.g., "white", "yellow")
            bg_color: Background color with alpha (e.g., "black@0.65")
        
        Returns:
            FFmpegResult with operation details
        """
        # Get video duration to calculate midpoint if needed
        try:
        ...
        position_map = {
            ...
            "bottom-left": f"x=10:y=h-text_h-10",
            "bottom-right": f"x=w-text_w-10:y=h-text_h-10"
        }
        pos = position_map.get(position, position_map["top-left"])
        
        # Escape special characters in text for FFmpeg
        # FFmpeg drawtext requires proper escaping
        escaped_text = text.replace("\\", "\\\\").replace(":", "\\:").replace("'", "\\'").replace("[", "\\[").replace("]", "\\]")
        
        # Build enable condition for time-based overlay
        enable_condition = ""
        if start_time is not None or end_time is not None:
            ...
            if conditions:
                enable_condition = f":enable='{'*'.join(conditions)}'"
        
        # Build drawtext filter with background box
        # Using drawtext with box for background
        # Note: FFmpeg will use default font if fontfile is not specified
        filter_complex = (
            f"drawtext=text='{escaped_text}':"
            f"fontsize={font_size}:"
            f"fontcolor={font_color}:"
            f"box=1:boxcolor={bg_color}:boxborderw=5:"
            f"{pos}{enable_condition}"
        )
        ...
        cmd = [
            ...
            "-i", input_path,
            "-vf", filter_complex,
            "-c:v", "libx264",  # Re-encode video to apply overlay
        ...
        logger.info(f"Add text overlay: duration={duration_ms:.0f}ms, size={filesize:,} bytes")
